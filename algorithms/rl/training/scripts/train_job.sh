#!/bin/bash
#SBATCH --job-name=rl_train
#SBATCH --output=training.out
#SBATCH --error=training.err
#SBATCH --time=72:00:00  # Increased time limit for CPU training
#SBATCH --mem=64G
#SBATCH --partition=work1
#SBATCH --cpus-per-task=8  # Using 8 CPU cores
#SBATCH --qos=normal

# Print start time and node information
echo "Job started at $(date)"
echo "Running on node: $(hostname)"
echo "Current directory: $(pwd)"

# Load modules
module load miniforge3

# Activate conda environment
source activate path_planning

# Set environment variables for memory management
export OMP_NUM_THREADS=8  # Match CPU cores
export MKL_NUM_THREADS=8  # Match CPU cores
export PYTHONUNBUFFERED=1  # Ensure Python output is not buffered

# Print environment information
echo "Python path: $(which python)"
echo "Using device: cpu"
echo "CPU cores: $(nproc)"

# Run training with unbuffered output
python -u -m algorithms.rl.train --agent both --episodes 1000 --grid_size 50 --num_agents 3 --num_goals 50 --num_obstacles 15 